...
export const StreakDisplay = () => {
  ...
  const getStreakIcon = () => {
    if (currentStreak >= 31) {
      return <Crown className="h-5 w-5 md:h-8 md:w-8 text-yellow-500 animate-bounce" />;
    } else if (currentStreak >= 8) {
      return <Trophy className="h-5 w-5 md:h-8 md:w-8 text-orange-500 animate-pulse" />;
    } else {
      return (
        <Flame
          className="h-5 w-5 md:h-8 md:w-8 text-red-500"
          style={{
  ...
  return (
    <Card className="p-3 md:p-6 space-y-2 md:space-y-4 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-950 dark:to-purple-950 animate-fadeIn">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-4">
          <div className="p-1 md:p-2 bg-white rounded-full shadow-lg dark:bg-gray-800">
            {getStreakIcon()}
          </div>
          <div>
            <h3 className="text-sm md:text-2xl font-bold font-heading">
        ...
        <div className="flex items-center gap-1 md:gap-4">
          <Badge variant="secondary" className="text-xs md:text-lg px-1.5 md:px-4 py-0.5 md:py-2">
            Level {level}
          </Badge>
          <div className="flex items-center gap-1 md:gap-2">
            <Gem className="h-3 w-3 md:h-5 md:w-5 text-blue-500" />
            <span className="text-xs md:text-base font-bold">{gems}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-1 md:space-y-2">
        ...
        <Progress value={((xpPoints % 100) / 100) * 100} className="h-1 md:h-2" />
      </div>
      ...
      <div className="flex items-center justify-between pt-1 md:pt-2">
        ...
        <Button
          ...
          className="flex items-center gap-1 md:gap-2 text-[10px] md:text-sm h-6 md:h-9"
        >
