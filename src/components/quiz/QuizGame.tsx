...
import { Trophy, Brain, Timer, Crown, Sparkles, Gem, Sword, Shield, Target, Star } from "lucide-react";
...
export const QuizGame = () => {
  ...
  const [totalCorrect, setTotalCorrect] = useState(() => {
    return parseInt(localStorage.getItem("totalCorrect") || "0");
  });
  const [totalAttempted, setTotalAttempted] = useState(() => {
    return parseInt(localStorage.getItem("totalAttempted") || "0");
  });
  ...
  const achievements = [
    ...
    },
    {
      ...
      unlocked: score === SAMPLE_QUESTIONS.length * 10,
    },
    {
      id: "streak-master",
      title: "Streak Master",
  ...
  }, [totalCorrect, totalAttempted, completedQuizzes]);
  ...
  const startGame = () => {
    ...
    setIsCorrect(null);
    setTotalAttempted(prev => prev + SAMPLE_QUESTIONS.length);
  };
